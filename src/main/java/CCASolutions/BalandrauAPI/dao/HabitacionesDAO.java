...
{
	@Query("SELECT h.nombre, h.descripcion, h.precioPorNoche, h.id FROM HabitacionesEntity h WHERE :numeroHuespedes <= h.numeroMaximoDeHuespedes AND h.id NOT IN (SELECT r.habitacion.id FROM ReservasEntity r WHERE r.fechaEntrada < :fechaSalida AND r.fechaSalida > :fechaEntrada)")
